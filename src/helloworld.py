...
def draw(da, ctx):
    ...
    #Gdk.cairo_set_source_rgba(ctx, Gdk.RGBA(0,1,0,1))
    #ctx.rectangle(0, 0, da.get_allocated_width(), da.get_allocated_height())
    #ctx.fill()
    #Gdk.cairo_set_source_pixbuf(ctx, display.get_cyl(ctx, 300, 200), 50, 20)
    #ctx.paint()
    #display.draw_end(ctx, None, 50, 20, 200)
    display.draw(da, ctx, (50, 20))
...
def main():
    ...
    color = Gdk.RGBA()
    color.parse("#43ACE2")
    #display = CylinderGenerator("pixmaps/LV.xpm",color)
    cylgen = CylinderGenerator("pixmaps/LV.xpm",color)
    display = DoubleCylinder(125, "this is name", "this is label upper", "this is label lower", 4, 300, 200)
    end_disp = End(cylgen)
    end_disp.set_height(200)
#    display.append_right(end_disp)
#    display.append_right(UnselectableSubcylinder("popup message", cylgen, 1, 100))
    display.set_height(200)
    #display.click((350, 150), True)
    #SingleCylinder(False, ''. "Here is my label", 4, 300, 200)
    win.show_all()
    Gtk.main()
